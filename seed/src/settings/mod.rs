use {
    crate::settings::{logic::LogicSettings, logic_mode::LogicMode},
    log::info,
    macros::fail,
...
impl Settings {
    ...
    pub fn validate(&self) -> Result<(), Box<dyn error::Error>> {
        // LC Requirement
        if !(0..=7).contains(&self.logic.lc_requirement) {
            fail!(
                "Invalid Lorule Castle Requirement: \"{}\" was not between 0-7, inclusive.",
                self.logic.lc_requirement
            );
        }

        // TODO - Yuganon Requirement
        ...
        if self.logic.yuganon_requirement != self.logic.lc_requirement {
            fail!(
        ...
        Different values for these settings are not yet supported!",
                self.logic.yuganon_requirement,
                self.logic.lc_requirement
            );
        }

        // Swords
        if self.logic.sword_in_shop && self.logic.swordless_mode {
            fail!("The sword_in_shop and swordless_mode settings cannot both be enabled.");
        }

        // Assured Weapons
        if self.logic.assured_weapon && (self.logic.sword_in_shop || self.logic.boots_in_shop) {
            fail!(
    ...
    pub fn log_settings(&self) {
        ...
        info!("Logic Mode:                     {}", match logic.logic_mode {
            crate::settings::logic_mode::LogicMode::Normal => "Normal",
            crate::settings::logic_mode::LogicMode::Hard => "Hard",
            crate::settings::logic_mode::LogicMode::Glitched => "Glitched",
            crate::settings::logic_mode::LogicMode::AdvGlitched => "Adv. Glitched",
            crate::settings::logic_mode::LogicMode::Hell =>
                "Hell - Did you really mean to choose this?",
            crate::settings::logic_mode::LogicMode::NoLogic => "No Logic",
        });
        info!(
            "Dungeon Prizes:                 {}",
