...
fn patch_prize_byaml(
    patcher: &mut Patcher, settings: &Settings, prizes: &DungeonPrizes,
) -> Result<()> {
...
fn patch_checks_unlocked_by_prizes(patcher: &mut Patcher, settings: &Settings) -> Result<()> {
...
/// Oren
fn patch_oren(patcher: &mut Patcher, settings: &Settings) -> Result<()> {
...
/// Irene
fn patch_irene(patcher: &mut Patcher, settings: &Settings) -> Result<()> {
    if !settings.logic.reverse_sage_events {
        return Ok(());
    }
...
/// Eastern Palace
fn patch_eastern(patcher: &mut Patcher, prize: Item) -> Result<()> {
...
/// House of Gales
fn patch_gales(patcher: &mut Patcher, prize: Item) -> Result<()> {
    // Debug stuff
...
/// Tower of Hera
fn patch_hera(patcher: &mut Patcher, prize: Item) -> Result<()> {
    // Debug stuff
...
/// Hyrule Castle
fn patch_hyrule_castle(patcher: &mut Patcher, prize: Item) -> Result<()> {
...
/// Dark Palace
fn patch_dark(patcher: &mut Patcher, prize: Item) -> Result<()> {
    // Debug stuff
...
/// Turtle Rock
fn patch_turtle(patcher: &mut Patcher, prize: Item) -> Result<()> {
    // Debug stuff
...
/// Desert Palace
fn patch_desert(patcher: &mut Patcher, prize: Item) -> Result<()> {
    // Debug stuff
...
/// Ice Ruins
fn patch_ice(patcher: &mut Patcher, prize: Item) -> Result<()> {
    // Debug stuff
...
fn modify_dungeon_reward(
    patcher: &mut Patcher, prize: Item, unq: u16, scene: game::Course, scene_index: u16,
    activate: bool, dest: Dest,
) -> Result<()> {
    let data = PrizePatchData::get(prize)?;
    patcher.modify_objs(
        scene,
