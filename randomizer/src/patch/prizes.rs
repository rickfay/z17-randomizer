...
use crate::filler::doors::Door::{
    DarkPalaceExit, DesertPalaceExit, EasternPalaceExit, HouseOfGalesExit, IceRuinsExit, SkullWoodsExit,
    SwampPalaceExit, TowerOfHeraExit, TurtleRockExit,
};
...
use crate::{
    DoorMap, MsbfKey, Patcher, SeedInfo,
...
pub(crate) fn patch_dungeon_prizes(patcher: &mut Patcher, seed_info: &SeedInfo, prizes: &DungeonPrizes) {
    ...
    patch_prize_byaml(patcher, seed_info, prizes);
}
...
fn patch_prize_byaml(patcher: &mut Patcher, seed_info: &SeedInfo, prizes: &DungeonPrizes) {
    patch_eastern(patcher, seed_info, prizes.ep_prize);
    patch_gales(patcher, seed_info, prizes.hg_prize);
    patch_hera(patcher, seed_info, prizes.th_prize);
    patch_dark(patcher, seed_info, prizes.pd_prize);
    patch_swamp(patcher, seed_info, prizes.sp_prize);
    patch_skull(patcher, seed_info, prizes.sw_prize);
    patch_thieves(patcher, prizes.tt_prize);
    patch_turtle(patcher, seed_info, prizes.tr_prize);
    patch_desert(patcher, seed_info, prizes.dp_prize);
    patch_ice(patcher, seed_info, prizes.ir_prize);
...
/// Eastern Palace
fn patch_eastern(patcher: &mut Patcher, seed_info: &SeedInfo, prize: Randomizable) {
...
/// House of Gales
fn patch_gales(patcher: &mut Patcher, seed_info: &SeedInfo, prize: Randomizable) {
    // Debug stuff
...
/// Tower of Hera
fn patch_hera(patcher: &mut Patcher, seed_info: &SeedInfo, prize: Randomizable) {
    // Debug stuff
    ...
    modify_dungeon_reward(patcher, prize, UNQ_PRIZE, DungeonHera, 1, false, sp);
    ...
    to right outside it. This is needed so that Dungeon ER doesn't make things weird. */

    // Disable old trigger
    patcher.modify_objs(DungeonHera, 1, [disable(872)]);

    // Add new trigger overlapping loading zone to Tower of Hera
    patcher.add_obj(
        FieldLight,
        3,
...
/// Dark Palace
fn patch_dark(patcher: &mut Patcher, seed_info: &SeedInfo, prize: Randomizable) {
    // Debug stuff
...
/// Turtle Rock
fn patch_turtle(patcher: &mut Patcher, seed_info: &SeedInfo, prize: Randomizable) {
    // Debug stuff
...
/// Desert Palace
fn patch_desert(patcher: &mut Patcher, seed_info: &SeedInfo, prize: Randomizable) {
    // Debug stuff
    ...
    let sp = if seed_info.settings.door_shuffle != DoorShuffle::Off {
        ...
        SpawnPoint::new(FieldDark, 31, 30) // Misery Mire Weather Vane
    };
...
/// Ice Ruins
fn patch_ice(patcher: &mut Patcher, seed_info: &SeedInfo, prize: Randomizable) {
    // Debug stuff
