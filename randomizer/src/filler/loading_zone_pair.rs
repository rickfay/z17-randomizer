...
macro_rules! loading_zones {
    (
        ...
        #[allow(unused)]
        impl Entrance {
            $(pub fn $func() -> (HashMap<Entrance, LoadingZone>, HashMap<Entrance, LoadingZone>) {
...
loading_zones! {
    dungeon_entrances {
        EASTERN_PALACE:
            entry(EasternRuinsUpper, FieldLight, 20, 36, 0)
            exit(EasternPalaceFoyer, DungeonEast, 1, 31, 0)
        HOUSE_OF_GALES:
            entry(HouseOfGalesIsland, FieldLight, 35, 43, 0)
            exit(HouseOfGalesFoyer, DungeonWind, 1, 305, 0)
        TOWER_OF_HERA:
            entry(TowerOfHeraEntrancePegs, FieldLight, 3, 14, 3)
            exit(TowerOfHeraFoyer, DungeonHera, 1, 758, 0)
        INSIDE_HYRULE_CASTLE:
            entry(HyruleCastleRoof, FieldDark, 0, 0, 0)
            exit(HyruleCastleDungeon, DungeonDark, 0, 0, 0)
        DARK_PALACE:
            entry(DarkPalaceWeatherVane, FieldDark, 0, 0, 0)
            exit(DarkPalaceFoyer, DungeonDark, 0, 0, 0)
        SWAMP_PALACE:
            entry(SwampPalaceAntechamber, CaveDark, 0, 0, 0)
            exit(SwampPalaceFoyer, DungeonWater, 0, 0, 0)
        SKULL_WOODS:
            entry(SkullWoodsOverworld, FieldDark, 0, 0, 0)
            exit(SkullWoodsFoyer, DungeonDokuro, 0, 0, 0)
        THIEVES_HIDEOUT:
            entry(LoruleCastleArea, FieldDark, 0, 0, 0)
            exit(ThievesHideoutB1, DungeonHagure, 0, 0, 0)
        TURTLE_ROCK:
            entry(TurtleRockFrontDoor, FieldDark, 0, 0, 0)
            exit(TurtleRockFoyer, DungeonKame, 0, 0, 0)
        DESERT_PALACE:
            entry(DesertPalaceWeatherVane, FieldLight, 31, 10, 2)
            exit(DesertPalaceFoyer, DungeonSand, 1, 38, 0)
        ICE_RUINS:
            entry(LoruleDeathEastTop, FieldLight, 0, 0, 0)
            exit(IceRuinsFoyer, DungeonIce, 0, 0, 0)
        LORULE_CASTLE:
            entry(LoruleCastleArea, FieldLight, 0, 0, 0)
            exit(LoruleCastle1F, DungeonGanon, 0, 0, 0)
    }
