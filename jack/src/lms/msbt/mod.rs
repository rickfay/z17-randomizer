...
impl MsbtFile {
    ...
    #[allow(unused)]
    pub fn add(&self, label: &str, message: &str) {
        unimplemented!();
    }

    fn get_item_index(&self, key: &str) -> Option<usize> {
        let hash = calc_lms_hash(String::from(key), self.lbl1.num_slots) as usize;
