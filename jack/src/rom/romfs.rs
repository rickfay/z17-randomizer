use {
    crate::JackFile,
    bytey::*,
    std::io::{prelude::*, ErrorKind::NotFound},
};
    ...
    R: Read + Seek,
{
    ...
    pub fn read<T>(&mut self, path: &str) -> Result<JackFile<T>>
    where
        T: From<Vec<u8>> + Into<Vec<u8>>
    {
        if path.ends_with('/') {
            ...
            let (offset, length) = self
                ...
                .ok_or_else(|| Error::new(NotFound, format!("File not found: '{}'.", path)))?;

            let bytes: Vec<u8> = read_slice_from_offset(
                &mut self.file,
                self.file_data + offset as u32,
                length as usize,
            )?.into();

            Ok(JackFile::new(&path, T::from(bytes)))
        }
    }
    ...
    fn find_dir(&mut self, name: Option<&str>, parent: u32) -> Result<u32> {
        if let Some(path) = name {
            ...
            Err(Error::new(NotFound, format!("Directory not found: '{}'.", path)))
        } else {
            Ok(parent)
        }
    }
