...
use std::path::Path;
use crate::item::Item::*;
use crate::rom::code::arm::*;
use crate::rom::exheader::ExHeader;
...
impl Code {
    ...
    pub fn dump<P>(self, path: P, exheader: &ExHeader) -> Result<(), Box<dyn error::Error>>
    where
        P: AsRef<Path>,
    {
        ...
        std::fs::write(path.join("exheader.bin"), exheader.as_ref())?;
        Ok(())
    }
}
...
impl Ips {
    ...
    pub fn write<W>(self, mut writer: W) -> Result<(), Box<dyn error::Error>>
    where
        W: Write,
    {
